...
def generate_time_series_folds(features_df: pd.DataFrame) -> List[Tuple[np.ndarray, np.ndarray]]:
    if 'month' not in features_df.columns:
        raise KeyError('features_df must contain a month column')
    ordered = features_df.sort_values(['month', 'sector_id']).reset_index(drop=False)
    month_values = pd.to_datetime(ordered['month'])
    folds: List[Tuple[np.ndarray, np.ndarray]] = []

    for train_start, train_end, val_start, val_end in FOLD_DEFINITIONS:
        ...
        train_idx = ordered.loc[train_mask, 'index'].to_numpy()
        valid_idx = ordered.loc[val_mask, 'index'].to_numpy()
