    ...
    records.append(
        {
            ...
            "sector_id": 2,
            "amount_new_house_transactions": 7.0,
            "search_kw_alpha": 2.0,
        }
    )
    records.append(
...
def test_projection_respects_cutoff():
    ...
    # Sector 1 has three historical values -> median fallback (source code 2)
    sector1_aug = projected[
        (projected["sector_id"] == 1) & (projected["month"] == pd.Timestamp("2024-08-01"))
    ]
    assert sector1_aug["amount_new_house_transactions"].iloc[0] == pytest.approx(12.0, rel=1e-6)
    assert (
        sector1_aug["amount_new_house_transactions_proj_source"].iloc[0] == 2
    )
    assert (
        sector1_aug["amount_new_house_transactions_proj_overridden"].iloc[0] == 0
    )
    ...
    assert sector1_sep["amount_new_house_transactions"].iloc[0] == pytest.approx(12.0, rel=1e-6)
    ...
    assert sector2_aug["amount_new_house_transactions"].iloc[0] == pytest.approx(9.0, rel=1e-6)
